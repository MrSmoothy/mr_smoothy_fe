    ...
    return (
      <div className="bg-[#F5EFE6] min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-[#4A2C1B] text-xl mb-4">ไม่พบสินค้า</p>
          <button
    ...
    <div className="bg-[#F5EFE6] min-h-screen py-12">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Left Side - Image */}
          <div className="bg-[#F5EFE6] rounded-lg border border-[#4A2C1B]/20 p-8">
            ...
            <div className="mt-6">
              <h1 className="text-3xl font-bold text-[#4A2C1B] mb-4">{drink.name.toUpperCase()}</h1>
              <div className="space-y-2">
                <p className="text-[#4A2C1B] font-medium">ส่วนผสมหลัก:</p>
                {drink.ingredients && drink.ingredients.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {drink.ingredients.map((ing, idx) => (
                      <span key={idx} className="text-[#4A2C1B]">
                        {ing.fruitName}
                  ...
                  <p className="text-[#4A2C1B]/70">ไม่มีข้อมูลส่วนผสม</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-4xl font-bold text-[#4A2C1B] mb-2">{drink.name}</h1>
              <p className="text-3xl font-bold text-[#4A2C1B]">{price} บาท</p>
            </div>

            {/* Main Ingredients */}
            <div className="border-t border-[#4A2C1B]/20 pt-6">
              <h2 className="text-xl font-bold text-[#4A2C1B] mb-4">Main Ingredients</h2>
              {drink.ingredients && drink.ingredients.length > 0 ? (
                <ul className="space-y-2">
                  {drink.ingredients.map((ing, idx) => (
                    <li key={idx} className="text-[#4A2C1B]">
            ...
            {/* Nutrition */}
            <div className="border-t border-[#4A2C1B]/20 pt-6">
              <h2 className="text-xl font-bold text-[#4A2C1B] mb-4">Nutrition</h2>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[#4A2C1B]/70">Calories</p>
                  <p className="text-[#4A2C1B] font-semibold">290 kcal</p>
                </div>
                <div>
                  <p className="text-[#4A2C1B]/70">Protein</p>
                  <p className="text-[#4A2C1B] font-semibold">12 g</p>
                </div>
                <div>
                  <p className="text-[#4A2C1B]/70">Fiber</p>
                  <p className="text-[#4A2C1B] font-semibold">6 g</p>
                </div>
                <div>
                  <p className="text-[#4A2C1B]/70">Sugar</p>
                  <p className="text-[#4A2C1B] font-semibold">10 g</p>
                </div>
              </div>
            </div>
            ...
            {drink.description && (
              <div className="border-t border-[#4A2C1B]/20 pt-6">
                <p className="text-[#4A2C1B]/70 text-sm mb-2">Allergens</p>
                <p className="text-[#4A2C1B]">{drink.description}</p>
              </div>
            )}

            {/* Add to Cart Section */}
            <div className="border-t border-[#4A2C1B]/20 pt-6 space-y-4">
              <div>
                <label className="block text-[#4A2C1B] font-semibold mb-2">เลือกขนาดแก้ว</label>
                <select
                  className="w-full rounded-md border border-[#4A2C1B]/30 px-4 py-3 text-[#4A2C1B] outline-none bg-[#F5EFE6]"
              ...
              <div className="flex items-center gap-4">
                <label className="text-[#4A2C1B] font-semibold">จำนวน:</label>
                <div className="flex items-center border border-[#4A2C1B]/30 rounded-md bg-[#F5EFE6]">
                  <button
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="p-3 text-[#4A2C1B] hover:bg-[#C9A78B] rounded-l-md transition-colors"
                  >
                    <Minus className="w-5 h-5" />
                  </button>
                  <span className="px-6 py-3 text-[#4A2C1B] font-semibold min-w-[60px] text-center">
                    {quantity}
                  </span>
                  <button
                    onClick={() => setQuantity(quantity + 1)}
                    className="p-3 text-[#4A2C1B] hover:bg-[#C9A78B] rounded-r-md transition-colors"
                  >
              ...
              <button
                ...
                className="w-full bg-black text-[#F5EFE6] px-6 py-4 rounded-md font-bold text-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
              >
